...
async function searchByNameAndBadge(searchTerm, badgeClass) {
    ...
    try {
        ...
        const response = await fetch(getApiUrl('products?start=0&limit=1000'));
        if (!response.ok) {
...
async function searchProducts(query) {
    ...
    try {
        ...
        const response = await fetch(getApiUrl(`products?search=${encodeURIComponent(currentSearchTerm)}&start=0&limit=1000`));
        const data = await response.json();
...
async function loadProducts(page = 0, append = false) {
    ...
    try {
        ...
        const response = await fetch(getApiUrl(`products?start=${start}&limit=30`));
...
async function searchStainlessSteelProducts() {
    ...
    try {
        ...
        const fetchedHtml = await Promise.all(pages.map(u => fetch(getProxyUrl(u), { cache: 'no-store' }).then(r => r.ok ? r.text() : '')));
